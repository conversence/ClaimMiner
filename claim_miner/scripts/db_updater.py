#!/usr/bin/env python3
"""
Copyright Conversence 2021-2024
License: Apache 2.0
"""
...
def declare_deploy(
    state: DbState, feature: str, prev_version: int, sha1sum: str, **conn_data
):
    psql_command(
...
def deploy(
    ...
    if needs_admin and not admin_conn_data:
        if dry_run or simulation:
            print(
                f"Note that the following: {needs_admin} would require the --admin-password flag"
            )
        else:
            print(
                f"I cannot run the following: {needs_admin} without the --admin-password flag."
            )
            return False
    ...
    if reversions and (dry_run or not simulation):
        ...
        for feature in reversions:
            ...
            if not dry_run:
                ...
                psql_command(
                    None, struct.revert, set_role_owner=not struct.head.admin, **f_conn
                )
        ...
        for struct, path, prev_version, sha1sum in calc_apply_target(
            ...
            if not dry_run:
                if not simulation:
                    f_conn = admin_conn_data if struct.head.admin else conn_data
                    psql_command(
                        None, path, set_role_owner=not struct.head.admin, **f_conn
                    )
...
def add_version(
    ...
    if requirements is not None or idempotent is not None or admin is not None:
        with open(struct.head.path) as f:
            ...
            for line in f:
                if (
                    REQUIRES_PATTERN.match(line)
                    or IDEMPOTENT_PATTERN.match(line)
                    or ADMIN_PATTERN.match(line)
                ):
                    is_after = True
                elif is_after:
                    after.append(line)
...
def main():
    ...
    add_versionp.add_argument(
        ...
        "--not-admin",
        action="store_true",
        help="does not require database admin privilege",
    )
    ...
    admin_conn_data = get_connection_data(
        ini_file, db, admin_password=args.admin_password or True, debug=args.debug
    )
    ...
    if args.command == "list":
        ...
        admin = True if args.admin else (False if args.not_admin else None)
        add_version(
            args.feature,
