...
class RoleData:
    ...
    def options(self) -> str:
        base = " ".join(
            f"{'' if getattr(self, k) else 'no'}{k}"
            for k in ("createrole", "inherit", "login")
        )
        if self.login:
...
def main():
    ...
    argp.add_argument(
        "--postgrest", action="store_true", help="create a postgrest config file"
    )
    ...
    argp.add_argument(
        ...
        help="A template configuration file with extra values",
    )
    ...
    if args.config_template:
        tmpl_file = ConfigParser()
        tmpl_file.read_file(args.config_template)
        for section in tmpl_file.sections():
            if not ini_file.has_section(section):
                ini_file.add_section(section)
            for option in tmpl_file.options(section):
                if ini_file.has_option(section, option):
                    continue
                value = tmpl_file.get(section, option)
                if value.startswith("RANDOM:"):
                    rlen = int(value.split(":")[1])
                    value = token_urlsafe(rlen)
                ini_file.set(section, option, value)
